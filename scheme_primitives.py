...
def primitive(*names, use_env=False):
    ...
    def add(fn):
        proc = PrimitiveProcedure(fn, use_env)
        for name in names:
...
@primitive("dotted-list?")
def scheme_dottedp(x):
    """Return whether x is a dotted-list"""
    if not isinstance(x, Pair):
        return True
    elif x.second is nil:
        return False
    else:
        return scheme_dottedp(x.second)
...
@primitive("set-car!")
def scheme_set_car(pair, x):
    check_type(pair, scheme_pairp, 0, 'set-car!')
    pair.first = x

@primitive("set-cdr!")
def scheme_set_car(pair, x):
    check_type(pair, scheme_pairp, 0, 'set-cdr!')
    pair.second = x
...
# Warning
###########
# Vector ops don't do type-checking for speed reasons !!

@primitive("make-vector")
def scheme_make_vector(k, fill=None):
    check_type(k, scheme_integerp, 0, 'make-vector')
    if k < 0:
        raise SchemeError("cannot make vector of size less than 0")
    return Vector([fill] * k)

@primitive("vector")
def scheme_vector(*vals):
    return Vector(vals)

@primitive("vector?")
def scheme_vectorp(x):
    return isinstance(x, Vector)

@primitive("vector-length")
def scheme_vector_length(vec):
    check_type(vec, scheme_vectorp, 0, 'vector-length')
    return len(vec)

def check_bounds(vec, k):
    vec_len = scheme_vector_length(vec)
    if k < 0 or k >= vec_len:
        raise SchemeError("index " + str(k) + " out of vector range")

@primitive("vector-ref")
def scheme_vector_ref(vec, k):
    check_type(vec, scheme_vectorp, 0, 'vector-ref')
    check_type(k, scheme_integerp, 1, 'vector-ref')
    check_bounds(vec, k)
    return vec[k]

@primitive("vector-set!")
def scheme_vector_set(vec, k, obj):
    check_type(vec, scheme_vectorp, 0, 'vector-set!')
    check_type(k, scheme_integerp, 1, 'vector-set!')
    check_bounds(vec, k)
    vec[k] = obj

@primitive("vector-copy")
def scheme_vector_copy(vec):
    check_type(vec, scheme_vectorp, 0, 'vector-copy')
    return Vector(vec)
...
def _tscheme_prep():
    ...
    if not _turtle_screen_on:
        ...
        turtle.mode('logo')
        turtle.Screen().tracer(100, 0)
        turtle.Screen().onclick(lambda x, y: exit())
        turtle.Screen().onkey(exit, "space")
...
def tscheme_color(c):
    ...
    if scheme_listp(c):
        ...
        for color in c:
            colors.append(color)
        turtle.color(colors)
    elif scheme_vectorp(c):
        turtle.color(c)
    elif scheme_numberp(c):
        turtle.color([c, c, c])
    else:
        c = eval(c)
        turtle.color(c)
